...
public class SoundExplorer implements MouseMotionListener, ActionListener, MouseListener, LineListener {
	...
	public SoundExplorer(SimpleSound sound, boolean inStereo) {
		...
		if (inStereo) leftSampleText = "Left (Top) Sample Value: ";
	...
	private void createWindow() {
		...
		if (fileName == null) fileName = "no file name";
	...
	private void createSoundPanel() {
		...
		if (inStereo) soundPanel.setLayout(new GridLayout(2, 1));
		else soundPanel.setLayout(new GridLayout(1, 1));
	...
	private void updateIndexValues() {
		...
		if (numSamplesPerPixelField != null) numSamplesPerPixelField.setText(Integer.toString((int) framesPerPixel));
		...
		try {
			...
			if (inStereo) rightSampleValue.setText(Integer.toString(sound.getRightSample(curFrame)));
		} catch (Exception ex) {
			catchException(ex);
		}
	}
	...
	private void setUpIndexPanel(JPanel indexPanel) {
		...
		prevButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (currentPixelPosition < 0) currentPixelPosition = 0;
				updateIndexValues();
				checkScroll();
				soundFrame.getContentPane().repaint();
			}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (DEBUG) System.out.println("mouse click:  " + currentPixelPosition);
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (mouseDragged) {
			...
			if (stopFrame >= sound.getLengthInFrames()) stopFrame = sound.getLengthInFrames();
	...
	public void checkScroll() {
		...
		if (sampleWidth != zoomOutWidth) {
			...
			if (currentPixelPosition < rectMinX || currentPixelPosition > rectMaxX) {
				...
				if (barXPos < 0) barXPos = 0;
				else if (barXPos > maxIndex) barXPos = maxIndex;
	...
	private void handleZoomIn(boolean checkScrollFlag) {
		...
		if (DEBUG) System.out.println("Zoom In:  currentPixelPosition = " + currentPixelPosition);
		...
		if (checkScrollFlag) checkScroll();
	}
	...
	private void handleZoomOut() {
		...
		if (DEBUG) System.out.println("Zoom Out:  currentPixelPosition = " + currentPixelPosition);
	...
	private class SamplingPanel extends JPanel {
		...
		public void createWaveForm(boolean forLeftSample) {
			...
			if (DEBUG) System.out.println("number of points: " + points.size());
			repaint();
	...
	public static void main(String args[]) {
		try {
			...
			SimpleSound   s    = new SimpleSound("bird_chirping2.wav");
